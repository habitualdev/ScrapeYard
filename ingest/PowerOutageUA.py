...
def get_records():
    web_records = requests.get("https://poweroutage.com/api/web/states?key=18561563181588&countryid=ua")
    return web_records.json()["WebStateRecord"]


class QueryClass:
    def __init__(self):
        with open("ingest.PowerOutageUA.lck") as f:
        ...
        while True:
            ...
            time.sleep(1)
            self.n += 1
            if not exists("ingest.PowerOutageUA.lck"):
                break
    ...
    def load_redis(self, parsed_yaml):
        ...
        for record in self.data:
            try:
                r.rpush("data", '{"Module":"PowerOutageUA", "Data": ' + str(record).replace("\'", "\"")[:-1] + ",\"TimeStamp\":\"" + str(time.time()) + '"}}')
            except:

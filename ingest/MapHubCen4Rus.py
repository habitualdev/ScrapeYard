...
def get_points():
    return requests.post("https://maphub.net/json/map_load/176607","{}").json()["geojson"]["features"]
...
class QueryClass:
    ...
    def load_redis(self, parsed_yaml):
        ...
        for record in self.data:
            try:
                r.rpush("data", '{"Module":"MapHubCen4Rus", "Data": ' + str(record).replace("\'", "\"")[:-1] + ",\"TimeStamp\":\"" + str(time.time()) + '"}}')
            except:
